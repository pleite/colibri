...
#ifdef _OPENMP
    /* The NPU backend uses the host CPU as a fallback execution path for the
     * fixed-shape matmul kernels. Use the priority order
     * COLI_NPU_THREADS -> COLI_CPU_THREADS -> COLI_THREADS -> OMP_NUM_THREADS
     * so NPU-specific settings win, but existing CPU thread controls still work. */
    const char *env = getenv("COLI_NPU_THREADS");
    if (!env || !*env) env = getenv("COLI_CPU_THREADS");
    if (!env || !*env) env = getenv("COLI_THREADS");
    if (!env || !*env) env = getenv("OMP_NUM_THREADS");
    int threads = 1;
    if (env && *env && coli_parse_positive_int(env, &threads)) {
        omp_set_dynamic(0);
        omp_set_num_threads(threads);
        return;
    }
    threads = coli_detect_thread_count();
    omp_set_dynamic(0);
    omp_set_num_threads(threads);
#endif
}
...
static int g_devices[COLI_NPU_MAX_DEVICES];
...
int coli_npu_mem_info(int device, size_t *free_bytes, size_t *total_bytes) {
    (void)device;
    if (free_bytes) *free_bytes = 16ULL * 1024ULL * 1024ULL * 1024ULL;
    if (total_bytes) *total_bytes = 16ULL * 1024ULL * 1024ULL * 1024ULL;
    return 1;
}
...
int coli_cuda_init(const int *devices, int count) {
    return coli_npu_init(devices, count);
}
...
int coli_cuda_device_count(void) {
    return coli_npu_device_count();
}

int coli_cuda_device_at(int index) {
    return coli_npu_device_at(index);
}

int coli_cuda_mem_info(int device, size_t *free_bytes, size_t *total_bytes) {
    return coli_npu_mem_info(device, free_bytes, total_bytes);
}

void coli_cuda_stats(int device, size_t *tensor_count, size_t *tensor_bytes) {
    coli_npu_stats(device, tensor_count, tensor_bytes);
}

int coli_cuda_tensor_upload(ColiCudaTensor **tensor,
                             const void *weights, const float *scales,
                             int fmt, int I, int O, int device) {
    return coli_npu_tensor_upload(tensor, weights, scales, fmt, I, O, device);
}
...
int coli_cuda_matmul(ColiCudaTensor **tensor,
                     ...
                     const void *weights, const float *scales,
                     int fmt, int S, int I, int O, int device) {
    return coli_npu_matmul(tensor, y, x, weights, scales, fmt, S, I, O, device);
}
...
size_t coli_cuda_tensor_bytes(const ColiCudaTensor *tensor) {
    return coli_npu_tensor_bytes(tensor);
}

int coli_cuda_tensor_device(const ColiCudaTensor *tensor) {
    return coli_npu_tensor_device(tensor);
}
