...
Backend runtime selection: when the full backend suite is built, the runtime dispatcher can be constrained at launch time with `COLI_RUNTIME_ENGINES` (comma-separated: `cpu,npu,vulkan,rocm,cuda`) and `COLI_RUNTIME_DISABLE_ENGINES` (same vocabulary). Example: `COLI_RUNTIME_ENGINES=cpu,vulkan ./glm ...` keeps CPU and Vulkan, while `COLI_RUNTIME_DISABLE_ENGINES=rocm,cuda ./glm ...` disables the GPU backends without changing the binary. For a pure CPU-only run, use `COLI_RUNTIME_ENGINES=cpu`; the special `none` token clears the requested backend mask, which still leaves CPU as the runtime fallback if no explicit backend is selected. Vulkan can also be disabled for an individual process with `COLI_VULKAN_DISABLE=1`.

ROCm versus Vulkan: ROCm is the best choice when the machine has an AMD GPU and the ROCm userspace is installed, because it gives native HIP execution and the closest match to the CUDA/ROCm backend path. Vulkan is the better fallback when you want a portable, Mesa-friendly backend on systems without ROCm, or when you need a single runtime path for AMD/Intel/Qualcomm hardware; it still preserves the same ABI through the runtime dispatcher, but the current implementation uses the CPU reference path unless the Vulkan loader and a compatible driver are available.
