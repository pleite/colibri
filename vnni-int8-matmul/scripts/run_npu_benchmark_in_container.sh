#!/bin/bash
#
# run_npu_benchmark_in_container.sh — Run NPU benchmark in the test container
#
# Uses the existing Dockerfile.strix-halo-test container which has all
# dependencies pre-installed (linker, kernel headers, Vulkan, etc.)

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
VNNI_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"
REPO_DIR="$(cd "${VNNI_DIR}/../.." && pwd)"  # colibri repo root
...
if ! podman image exists "${HARNESS_IMAGE}" 2>/dev/null; then
    ...
    echo "Harness image already exists: ${HARNESS_IMAGE}"
fi
...
if [[ -e "${NPU_DEVICE}" ]]; then
    echo "NPU device found: ${NPU_DEVICE}"
    NPU_ARGS+=(--device "${NPU_DEVICE}")
else
    echo "WARNING: ${NPU_DEVICE} not found - NPU tests will be skipped"
fi
...
cat > "${TEMP_DIR}/npu_benchmark_test.c" << 'EOF'
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdint.h>
...
echo "Running NPU benchmark inside container..."
echo "==========================================="
echo ""
...
podman run --rm \
    ...
    --volume "${TEMP_DIR}:/tmp:ro" \
    --workdir /work/vnni-int8-matmul \
    "${HARNESS_IMAGE}" bash -lc '
set -euo pipefail
...
# Copy test file from host
cp /tmp/npu_benchmark_test.c /work/vnni-int8-matmul/npu_benchmark_test.c

# Build the benchmark
gcc -I. -I.. -O3 -march=native -std=c11 -o /tmp/npu_benchmark_test \
    /work/vnni-int8-matmul/npu_benchmark_test.c \
...
podman cp "${CONTAINER_NAME}:/work/npu_benchmark_results.txt" "${VNNI_DIR}/npu_benchmark_results.txt" 2>/dev/null || true

echo ""
echo "=== Benchmark Complete ==="
echo "Results: ${VNNI_DIR}/npu_benchmark_results.txt"
echo ""
echo "To view results:"
echo "  cat ${VNNI_DIR}/npu_benchmark_results.txt"
