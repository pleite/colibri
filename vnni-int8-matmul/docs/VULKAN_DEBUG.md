...
The Vulkan backend in `vnni-int8-matmul/gpu/vulkan_backend.c` is tested only on Strix Halo. The harness in `vnni-int8-matmul/scripts/strix-halo-podman-test.sh` is intentionally explicit about the display requirement and does not attempt portable fallbacks.

## Prerequisites

The Strix Halo test harness requires:

- a live graphical session on the host (`DISPLAY` or `WAYLAND_DISPLAY` must be set)
- the AMD RADV ICD at `/usr/share/vulkan/icd.d/radeon_icd.x86_64.json`
- access to `/dev/dri` and `/dev/kfd` from Podman
- the `docker.io/kyuz0/amd-strix-halo-toolboxes:vulkan-radv` container image

If the host is running in a TTY or SSH-only session, the harness exits immediately with an error instead of falling back to a portable path.
